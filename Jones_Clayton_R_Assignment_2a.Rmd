---
title: 'Jones_Clayton Programming with R Assignment #2a (75 points)'
output:
...
np<-n*p
sd<-sqrt(n*p*(1-p))
pnorm(25.5, mean = np, sd = sd) - pnorm(24.5, mean = np, sd = sd)
...
n<-100
np<-n*p
barplot(dpois(0:10, lambda = np), names.arg = c(0:10), col = "blue", xlab = "Number of Successful Outcomes", main = "Poisson Distribution")
barplot(dbinom(0:10, size = n, prob = p),names.arg = c(0:10), col = "red", xlab = "Number of Successful Outcomes", main = "Binomial Distribution")
```
...
(ev<-round(sum(outcomes*probs),digits = 2))
(sd<-round(sum(probs*(outcomes-ev)^2),digits = 2))
...
plot(x = outcomes, y = cumprobs, type = "s", xlab = "Outcomes", ylab = "Cumulative Probability")
abline(v = median(outcomes), lty = 1, col = "red")
text(x = 3.5, y = .4, labels = "Median = 3", col = "red")
...
str(faithful)
summary(faithful)
hist(faithful$waiting, xlab = "Waiting Times")

sum(faithful$eruptions < 3 & faithful$waiting > 70)/sum(faithful$waiting > 70)
```
...
```{r test2ai}
(val<-faithful[(faithful$eruptions < 3 & faithful$waiting > 70),])
plot(x = faithful$waiting, y = faithful$eruptions, xlab = "Waiting Time", ylab = "Eruption Time", main = "Eruption Time vs Waiting Time", pch = 16, col = "orange")
abline(v = 70)
abline(h = 3)
points(val[,2],val[,1], col = "purple", pch = 16, cex = 1.5)
```

(ii) What does the plot suggest about the relationship between eruption time and waiting time?

***Answer: The plot suggests that there is a positive, linear relationship between eruption time and waiting time. As waiting time increases, so does eruption time.    ***
...
```{r test2b}
consec<-matrix(data = faithful$waiting, ncol = 2, byrow = TRUE)
plot(x = consec[,1], y = consec[,2], xlab = "First Waiting Time", ylab = "Second Waiting Time", main = "Plot of Consecutive Waiting Times", col = "blue", pch = 16)
...
```{r test2c}
cor.test(x = consec[,1], y = consec[,2], alternative = "two.sided", method = "kendall", conf.level = .95)
...
```{r test3a}
par(mfrow = c(1,2))
dietone<-subset(result, Diet == 1)
boxplot(dietone$weight, main = "Diet 1", ylab = "Weight", col = "blue")
dietthree<-subset(result, Diet == 3)
boxplot(dietthree$weight, main = "Diet 3", ylab = "Weight", col = "green")
```
...
```{r test3b}
t.test(result$weight ~ result$Diet, var.equal = TRUE)
```
...
# There are multiple ways to approach the subsetting task. The method you choose is up
# to you.
day20<-subset(ChickWeight, Diet == 3 & Time == 20)
day21<-subset(ChickWeight, Diet == 3 & Time == 21)
...
```{r test3c}
plot(x = day20$weight, y = day21$weight, xlab = "Weights for Time = 20", ylab = "Weights for Time = 21", main = "Weights for Time = 21 vs Weights for Time = 20", pch = 16)
abline(a = 0, b = 1)
```
...
```{r test3d}
gain<-day21$weight - day20$weight
n<-length(gain)
sd<-sd(gain)
mu<- mean(gain)
...
cv<-qt(p = .95, df = n-1)
cat("\nCritical Value =", cv)
pv<- pt(t, df = n - 1, lower = FALSE)
cat("\nP-Value =", pv)
lowval<-mu-cv*(sd/sqrt(n))
cat("\nCI = (",lowval,", infinity )")
```
...
kurtosis(Nile)
par(mfrow = c(1, 2))
qqnorm(Nile, col = "blue", main = "QQ Plot")
qqline(Nile)
boxplot(Nile, col = "blue", main = "Box Plot")
```
...
for (i in 1:1000) {
  dummy<-sample(Nile, 16, replace = TRUE)
  sample1[i] = mean(dummy)
}
set.seed(127)
sample2<-numeric(0)
for (i in 1:1000) {
  dummy<-sample(Nile, 64, replace = TRUE)
  sample2[i] = mean(dummy)
}
...
a<-rbind(c(s1mean, s1sd, s1var),c(s2mean,s2sd,s2var))
colnames(a) <- c("Average", "Std Dev", "Variance")
...
hist(sample1, xlim = c(750,1050), col = "blue", freq = FALSE, ylim = c(0, 0.025))
curve(dnorm(x, mean = s1mean, sd = s1sd), col = "orange", lwd = 2, add = TRUE)
hist(sample2, xlim = c(750,1050), col = "red", freq = FALSE, ylim = c(0, 0.025))
curve(dnorm(x, mean = s2mean, sd = s2sd), col = "orange", lwd = 2, add = TRUE)
```
...
hist(warpbreaks$breaks, xlab = "Breaks", main = "Histogram of Warp Breaks")
...
```{r test5b}
chisq.test(x = table(warpbreaks$tension, warpbreaks$number),correct = FALSE)
cat("With a P value of 0.01064, we can reject the null hypothesis, indicating that the variables are not independent.")
```
...
(val<-sum((tbl[1,1]-e11)**2/e11,(tbl[1,2]-e12)**2/e12,(tbl[2,1]-e21)**2/e21,(tbl[2,2]-e22)**2/e22,(tbl[3,1]-e31)**2/e31,(tbl[3,2]-e32)**2/e32))
...
chisq_function <- function(x) {
  ...
  chi_sq_val<-sum((tbl[1,1]-e11)**2/e11,(tbl[1,2]-e12)**2/e12,(tbl[2,1]-e21)**2/e21,(tbl[2,2]-e22)**2/e22,(tbl[3,1]-e31)**2/e31,(tbl[3,2]-e32)**2/e32)
  
  # Code for calculating the degrees of freedom and p-value
  degf<-(ncol(x)-1)*(nrow(x)-1)
  p_val <- pchisq(chi_sq_val, df = degf, lower.tail = FALSE)
    
  # Code to ouput the chi-squared, degrees of freedom and p-value 
  cat("Chi-squared value:",chi_sq_val,"\nDegrees of Freedom:",degf,"\nP-Value:",p_val,"\n")
}
